...
<body>
	<form action="conf/doBpointEdit" name="usertypeaddForm"
		id="usertypeaddForm" method="post">
		<input name="id" id="id" value="${priceInfo.id}" type="hidden" /> <input
			name="priceId" id="priceId" value="${priceInfo.priceId}"
			type="hidden" />
		<div id="zhongxin">
			<table id="table_report"
						...
						style="vertical-align: top; width: 120px;">
							...
							<c:forEach items="${appnames}" var="name">
								<option value="${name.appId }"
									<c:if test="${priceInfo.appId == name.appId}">selected="selected"</c:if>>${name.appName}</option>
							</c:forEach>
					</select></td>
				</tr>
				<tr>
					...
					<td><input type="text" name="priceName" id="priceName"
						autocomplete="off" value="${priceInfo.priceName}" maxlength="128"
				...
				<tr>
					...
					<td><textarea class="form-control limited" id="form-field-9"
							...
							title="计费点功能简介">${priceInfo.priceBriefing}</textarea></td>
				</tr>
				<tr id="tr_priceType">
					<th>计费点类型：</th>
					<td><input name="priceType" type="radio" class="ace" value="1"
						<c:if test="${priceInfo.priceType == 1}">checked="checked"</c:if> />
						<span class="lbl">固定</span> <input name="priceType" type="radio"
						class="ace" value="2"
						<c:if test="${priceInfo.priceType == 2}">checked="checked"</c:if> />
					...
					<td style="vertical-align: top;"><select class="chzn-select"
						...
						style="vertical-align: top; width: 120px;">
							...
							<c:forEach items="${priceservices}" var="priceservice">
								<option value="${priceservice.name }"
									<c:if test="${priceInfo.priceService == priceservice.code}">selected="selected"</c:if>>${priceservice.name}</option>
							</c:forEach>
					</select></td>
				</tr>
				<tr id="tr_country">
						...
						style="vertical-align: top; width: 120px;">
							...
							<c:forEach items="${countrys}" var="country">
								<option value="${country.code }"
									<c:if test="${priceInfo.country == country.code}">selected="selected"</c:if>>${country.name}</option>
							</c:forEach>
					</select></td>
				</tr>
				<tr id="tr_operator">
						...
						style="vertical-align: top; width: 120px;">
							...
							<c:forEach items="${operators}" var="operator">
								<option value="${operator.code }"
									<c:if test="${priceInfo.operator == operator.code}">selected="selected"</c:if>>${operator.name}</option>
							</c:forEach>
					</select></td>
				</tr>
				<tr id="tr_localPrice">
					...
					<td style="vertical-align: top;"><select class="chzn-select"
						...
						style="vertical-align: top; width: 120px;">
							<option value="">-请选择-</option>
							<c:forEach items="${localPrices}" var="localPrice">
								<option value="${localPrice.code }"
									<c:if test="${priceInfo.qcId == localPrice.code}">selected="selected"</c:if>>${localPrice.name}</option>
							</c:forEach>
					</select></td>
				</tr>
				<tr id="tr_globalPrice">
					...
					<td style="vertical-align: top;"><input type="text"
						name="globalPrice" id="globalPrice" autocomplete="off"
						value="${priceInfo.globalPrice}" maxlength="128"
						placeholder="请在这里输入全球价格" title="全球价格" /></td>
				</tr>
				<tr>
					<th>是否显示提示框：</th>
					<td><input name="isTip" type="radio" class="ace" value="1"
						<c:if test="${priceInfo.isTip == 1}">checked="checked"</c:if> />
						<span class="lbl">是</span> <input name="isTip" type="radio"
						class="ace" value="2"
						<c:if test="${priceInfo.isTip == 2}">checked="checked"</c:if> />
						<span class="lbl">否</span></td>
				</tr>
				<tr id="tr_isParam">
					<th>是否有额外参数：</th>
					<td><input name="isParam" type="radio" class="ace" value="1"
						<c:if test="${priceInfo.isParam == 1}">checked="checked"</c:if> /><span
						class="lbl">是</span> <input name="isParam" type="radio"
						class="ace" value="2"
						<c:if test="${priceInfo.isParam == 2}">checked="checked"</c:if> />
						<span class="lbl">否</span></td>
				</tr>
				<tr id="tr_param">
					<th>额外参数：</th>
					<td style="vertical-align: top;"><select class="chzn-select"
						...
						style="vertical-align: top; width: 120px;">
							<option value="">-请选择-</option>
							<option value="1"
								<c:if test="${priceInfo.params == 1}">selected="selected"</c:if>>向上原则</option>
							<option value="2"
								<c:if test="${priceInfo.params == 2}">selected="selected"</c:if>>向下原则</option>
					</select></td>
				</tr>
				<tr>
	...
	<script type="text/javascript">
		...
		var type = ${priceInfo.priceType};
		var is_param = ${priceInfo.isParam};
		if(type == 1){
			//固定
		...
		$("#operator").change(function() {
            ...
            var url ="<%=basePath%>conf/getLocalPrice?priceService="
					...
					$.get(url, function(data) {
						var jsonObj = eval('(' + data + ')');
						$("#qcId").append("<option value='' >-请选择-</option>");
						for ( var p in jsonObj) {
							var code = (jsonObj[p].name);
							$("#qcId").append(
									"<option value=" + code + ">"
